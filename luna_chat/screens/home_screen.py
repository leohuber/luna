...
"""
    ...
    BINDINGS = [
        ...
        Binding(
            ...
            "Options",
            key_display="^o",
            tooltip="Change the model, system prompt, and check where Luna" " is storing your data.",
        ),
    ]
    ...
    async def create_new_chat(self, event: PromptInput.PromptSubmitted) -> None:
        text = event.text
        await self.luna.launch_chat(  # type: ignore
            prompt=text,
            model=self.luna.runtime_config.selected_model,
        )
