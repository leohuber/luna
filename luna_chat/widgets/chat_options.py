...
from luna_chat.database.database import sqlite_file_name
...
class OptionsModal(ModalScreen[RuntimeConfig]):
    ...
    def __init__(
        ...
        self.runtime_config = self.luna.runtime_config
    ...
    def compose(self) -> ComposeResult:
        with VerticalScroll(id="form-scrollable") as vs:
            ...
            with RadioSet(id="available-models") as models_rs:
                ...
                for model in self.luna.launch_config.all_models:
    ...
    def update_state(self, event: TextArea.Changed | RadioSet.Changed) -> None:
        ...
        self.luna.runtime_config = self.luna.runtime_config.model_copy(
            update={
    ...
    def apply_overridden_subtitles(self, system_prompt_ta: TextArea, selected_model_rs: RadioSet) -> None:
        if (
            self.luna.launch_config.default_model != self.luna.runtime_config.selected_model.id
            and self.luna.launch_config.default_model != self.luna.runtime_config.selected_model.name
        ):
            selected_model_rs.border_subtitle = "overrides config"
        else:
            selected_model_rs.border_subtitle = ""

        if system_prompt_ta.text != self.luna.launch_config.system_prompt:
